import { getLatestCboeClose } from "./cboe";
import { getNextEarningsDate } from "./earnings-calendar";
import { getMacroEvents, type MacroEvent } from "./macro-calendar";
import {
  getDailyCandles,
  getLatestFiling,
  getQuotes,
...
function buildVci(vix: number, vixHistory: number[], vvix: number, vix3m: number) {
  ...
  const conclusion: VciConclusion =
...
function eventLabelFromKind(kind: MacroEventKind, _locale: Locale) {
  if (kind === MACRO_EVENT_KIND.FOMC) return "FOMC";
  if (kind === MACRO_EVENT_KIND.CPI) return "CPI";
  return "PPI";
}
...
export async function buildSellPutReport(symbol: string, locale: Locale = "zh"): Promise<SellPutReport> {
  ...
  const trendLabel: TrendLabel = distanceToMa120 >= 0 ? TREND_LABEL.ABOVE_MA : TREND_LABEL.BELOW_MA;
